...
public class BirtDesignEngineApiComplete {
	...
	public static void main(String[] args) {
		try {
			BirtDesignEngineApiComplete de = new BirtDesignEngineApiComplete();
			de.buildReport();
		} catch (IOException e) {
	...
	void buildDataSource() throws SemanticException {
		...
		// PropertyHandle ph =
		// designHandle.getPropertyHandle("propertyBindings");
		// ph.addItem(pb);
	...
	void buildJointDataSet() throws SemanticException {
		...
		PropertyHandle conditionHandle = jds.getPropertyHandle(JointDataSet.JOIN_CONDITONS_PROP);
	...
	void addSortKey(TableHandle th) {
		try {
			...
			sk.setKey(
					"if( params[\"srt\"].value){ if( params[\"srt\"].value == 'a' ){	row[\"CustomerName\"]; }else{ row[\"CustomerCity\"];}}");
	...
	void addHyperlink(LabelHandle lh) {
		try {
			...
			/*
			 * <structure name="action"> <property
			 * name="linkType">drill-through</property> <property
			 * name="reportName">detail.rptdesign</property> <property
			 * name="targetWindow">_blank</property> <property
			 * name="targetFileType">report-design</property> <list-property
			 * name="paramBindings"> <structure> <property
			 * name="paramName">order</property> <expression
			 * name="expression">row["ORDERNUMBER"]</expression> </structure>
	...
	void buildReport() throws IOException, SemanticException {
		...
		try {
			...
			// formatValueToSet );
			// has to have a style
			// data.getStyle().setDateTimeFormat("dd/mm/yy");
